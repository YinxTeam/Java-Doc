...
[05.JVM_什么是编译和反编译](../05.JVM/什么是编译和反编译.md)
...
![image.png](./assets/javap命令的作用是什么_-1.png)
...
一般情况下我们会用到javap命令的时候不多，一般只有在真的需要看字节码的时候才会用到。但是字节码中间暴露的东西是最全的，你肯定有机会用到，比如我在分析synchronized的原理的时候就有是用到javap。通过javap生成的字节码，我发现synchronized底层依赖了ACC\_SYNCHRONIZED标记和monitorenter、monitorexit两个指令来实现同步。
