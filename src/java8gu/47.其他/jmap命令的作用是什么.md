...
**查看java 堆（heap）使用情况**，执行命令： hollis@hos:~/workspace/design\_apaas/apaasweb/control/bin$ jmap -heap 31846
...
**查看堆内存(histogram)中的对象数量及大小**。执行命令： hollis@hos:~/workspace/design\_apaas/apaasweb/control/bin$ jmap -histo 3331
...
**将内存使用的详细情况输出到文件**，执行命令： hollis@hos:~/workspace/design\_apaas/apaasweb/control/bin$ jmap -dump:format=b,file=heapDump 6900
