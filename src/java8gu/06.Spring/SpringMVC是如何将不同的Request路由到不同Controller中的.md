---
title: "SpringMVC是如何将不同的Request路由到不同Controller中的"
author: Hollis
category:
  - Java八股文
---
...
interface RequestCondition {
	#combine(T): T
	#compareTo(T, HttpServletRequest): int
}
...
class RequestMappingInfo {
  ...
  *paramsCondition: ParamsRequestCondition
  *headersCondition: HeadersRequestCondition
  *consumesCondition: ConsumesRequestCondition
  *producesCondition: ProducesRequestCondition
  *customConditionHolder: RequestConditionHolder
}
class MappingRegistry {
	*registry: Map<RequestMappingInfo, MappingRegistration>
  *pathLookup: Map<String, RequestMappingInfo>
  *nameLookup: Map<String, List<HandlerMethod>>
...
HandlerMapping *-- MappingRegistry
MappingRegistry *-- MappingRegistration
MappingRegistry *-- RequestMappingInfo
MappingRegistration *-- HandlerMethod
...
![图表](./assets/SpringMVC是如何将不同的Request路由到不同Controller中的_-1.svg)
...
![image.png](./assets/SpringMVC是如何将不同的Request路由到不同Controller中的_-2.png)