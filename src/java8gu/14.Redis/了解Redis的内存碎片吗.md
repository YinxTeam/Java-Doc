...
-   **used\_memory**：表示Redis为了保存数据实际申请使用的内存空间。
-   **used\_memory\_rss**：表示操作系统实际分配给Redis的物理内存空间，其中包含了内存空间碎片。
-   **mem\_fragmentation\_ratio**：表示Redis当前的内存碎片率。等于used\_memory\_rss/used\_memory
