---
title: "ES_不支持_decimal,如何避免丢失精度"
author: Hollis
category:
  - Java八股文
---

# ES_不支持_decimal,如何避免丢失精度
...
[20.ElasticSearch_ES支持哪些数据类型,和MySQL之间的映射关系是怎么样的](./ES支持哪些数据类型,和MySQL之间的映射关系是怎么样的.md)
...
Elasticsearch 的 scaled\_float 类型是一种数值数据类型，专门用于存储浮点数。其特点是通过一个缩放因子（scaling factor）将浮点数转换为整数来存储，从而在一定范围内提高存储和计算的效率。
