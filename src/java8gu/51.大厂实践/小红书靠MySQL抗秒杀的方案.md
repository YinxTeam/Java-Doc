...
![](./assets/小红书靠MySQL抗秒杀的方案-3.png)

测试数据均为sysbench模拟标准库存扣减模型进行的压测数据。

![](./assets/小红书靠MySQL抗秒杀的方案-4.png)

首先对秒杀场景进行分析，抽象出了它的事务模型。下面所示为最经典的库存扣减模型：

1.  begin;
2.  insert into inventory\_log value (...);

\-- 插入库存修改的流水表

3.  update inventory set quantity=quantity-1 where sku\_id=? and quantity `>` 0;
...
![](./assets/小红书靠MySQL抗秒杀的方案-5.png)

![](./assets/小红书靠MySQL抗秒杀的方案-6.jpeg)
...
![](./assets/小红书靠MySQL抗秒杀的方案-7.jpeg)

![](./assets/小红书靠MySQL抗秒杀的方案-8.png)
...
![](./assets/小红书靠MySQL抗秒杀的方案-9.png)

![](./assets/小红书靠MySQL抗秒杀的方案-10.png)

![](./assets/小红书靠MySQL抗秒杀的方案-11.png)

左右滑动查看

![](./assets/小红书靠MySQL抗秒杀的方案-12.png)
...
![](./assets/小红书靠MySQL抗秒杀的方案-18.png)

**4.3** **Binlog并行提交**

整个组的Binlog是同一批由Leader统一提交。

![](./assets/小红书靠MySQL抗秒杀的方案-19.png)
...
![](./assets/小红书靠MySQL抗秒杀的方案-20.png)

![](./assets/小红书靠MySQL抗秒杀的方案-21.jpeg)

左右滑动查看

对于以上两种场景，Crash Recovery需要做好兼容处理。具体的思路如下:

![](./assets/小红书靠MySQL抗秒杀的方案-22.png)

以数据组为单位进行扣减数据的提交和回滚，相对事务粒度更大一些。

![](./assets/小红书靠MySQL抗秒杀的方案-23.png)

正如上面所述，在内核中使用的Hint关键字如下：

![](./assets/小红书靠MySQL抗秒杀的方案-24.png)

![](./assets/小红书靠MySQL抗秒杀的方案-25.png)
