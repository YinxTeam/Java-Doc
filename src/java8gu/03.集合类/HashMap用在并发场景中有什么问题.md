...
![hashmap-recycle.drawio.png](./assets/HashMap用在并发场景中有什么问题_-1.png)
...
![image.png](./assets/HashMap用在并发场景中有什么问题_-2.png)

此时thread-2开始执行，当thread-2扩容完成后，结果如下：

![image.png](./assets/HashMap用在并发场景中有什么问题_-3.png)

此时thread-1抢占到执行时间，开始执行：`e.next = newTable[i]; newTable[i] = e; e = next;`后，会变成如下样式：

![image.png](./assets/HashMap用在并发场景中有什么问题_-4.png)

接着，进行下一次循环，继续执行`e.next = newTable[i]; newTable[i] = e; e = next;`，如下图所示

![image.png](./assets/HashMap用在并发场景中有什么问题_-5.png)

因为此时e!=null，且e.next = null，开始执行最后一次循环，结果如下：

![image.png](./assets/HashMap用在并发场景中有什么问题_-6.png)
