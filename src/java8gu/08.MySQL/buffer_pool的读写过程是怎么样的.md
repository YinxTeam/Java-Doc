---
title: "buffer_pool的读写过程是怎么样的"
author: Hollis
category:
  - Java八股文
---

# buffer_pool的读写过程是怎么样的
...
[08.MySQL_什么是buffer_pool](./什么是buffer_pool.md)
...
![](./assets/buffer_pool的读写过程是怎么样的_-1.png)（图源自：[08.MySQL_InnoDB的一次更新事务过程是怎么样的](./InnoDB的一次更新事务过程是怎么样的.md)）

当脏页的百分比达到innodb\_max\_dirty\_pages\_pct\_lwm变量定义的低水位标记时，将启动缓冲池刷新。缓冲池页的默认低水位标记为10%。innodb\_max\_dirty\_pages\_pct\_lwm值设为0会禁用这种提前刷新行为。

InnoDB还使用了一种适应性刷新算法，根据redo log的生成速度和当前的刷新率动态调整刷新速度。其目的是通过确保刷新活动与当前工作负载保持同步，来平滑整体性能。

当然，我们也可以手动触发脏页的刷新到磁盘，例如通过执行SET GLOBAL innodb\_buffer\_pool\_dump\_now=ON 来进行一次脏页刷新。
