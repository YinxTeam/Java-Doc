...
![image.png](./assets/Innodb的RR到底有没有解决幻读_-1.png)
...
![image.png](./assets/Innodb的RR到底有没有解决幻读_-2.png)

像上面这种情况，在RR的级别下，当我们使用SELECT … FOR UPDATE的时候，会进行加锁，不仅仅会对行记录进行加锁，还会对记录之间的间隙进行加锁，这就叫做间隙锁。

[08.MySQL_MySQL的行级锁锁的到底是什么](./MySQL的行级锁锁的到底是什么.md)
...
![image.png](./assets/Innodb的RR到底有没有解决幻读_-3.png)
...
![image.png](./assets/Innodb的RR到底有没有解决幻读_-4.png)
...
![](./assets/Innodb的RR到底有没有解决幻读_-5.jpeg)图源：MySQL 8.0 Reference Manual
