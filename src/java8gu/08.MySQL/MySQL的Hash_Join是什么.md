...
[08.MySQL_为什么大厂不建议使用多表join](./为什么大厂不建议使用多表join.md)
...
![image.png](./assets/MySQL的Hash_Join是什么_-1.png)

探测阶段，在这个过程中，从school表中取出记录之后，去hash表中查询，找到匹配的数据，在做聚合就行了。

![image.png](./assets/MySQL的Hash_Join是什么_-2.png)

需要注意的时候，上面的Hash表是在内存中的，但是，内存是有限的（通过join\_buffer\_size限制），如果内存中存不下驱动表的数据怎么办呢？
