...
在不考虑索引合并的情况下，他可能会用到customer\_name的索引，也可能会用到age的索引。那么，当我们分析两个索引的好坏的时候，有两种非常直观的方式：
...
[08.MySQL_MySQL_中如何查看一个_SQL_的执行耗时](./MySQL_中如何查看一个_SQL_的执行耗时.md)
...
-   type 字段表示查询的访问类型。**理想情况下，类型应该是 const、eq\_ref 或 ref**，这些类型的查询效率较高。如果是 ALL 或 index，说明进行的是全表扫描或全索引扫描，这通常不是最优的选择。
