...
储备知识1：MySQL数据库中的每行记录中，除了保存了我们自己定义的一些字段以外，还有一些重要的隐式字段的：db\_row\_id（隐藏主键，如果我们没有给这个表创建主键，那么会以这个字段来创建聚簇索引。）、 db\_trx\_id（对这条记录做了最新一次修改的事务的ID）、以及 db\_roll\_ptr（回滚指针，指向这条记录的上一个版本，其实他指向的就是Undo Log中的上一个版本的快照的地址。 ）

储备知识2：在MVCC中，隐藏字段中的db\_roll\_ptr用来构建版本链，db\_trx\_id也是一个重要的用来判断快照可见性的一个字段。
...
[08.MySQL_什么是回表,怎么减少回表的次数](./什么是回表,怎么减少回表的次数.md)
