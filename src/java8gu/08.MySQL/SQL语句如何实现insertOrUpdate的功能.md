...
![image.png](./assets/SQL语句如何实现insertOrUpdate的功能_-1.png)

再执行一次：

![image.png](./assets/SQL语句如何实现insertOrUpdate的功能_-2.png)

此时，由于 `value` 列存在唯一键约束，并且已经存在一条记录 `value = 'a'`，所以不会插入新记录，而是会执行更新操作。但即便如此，自增主键 `id` 的计数器依然会增加。

然后再插入一条新的记录：

![image.png](./assets/SQL语句如何实现insertOrUpdate的功能_-3.png)
