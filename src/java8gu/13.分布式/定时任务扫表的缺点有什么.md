...
[13.分布式_如何基于本地消息表实现分布式事务](./如何基于本地消息表实现分布式事务.md)
...
[08.MySQL_区分度不高的字段建索引一定没用吗](../08.MySQL/区分度不高的字段建索引一定没用吗.md)
...
这个做法，有个小问题，那就是INIT的数据的ID可能不是连续的，那么就需要考虑其他的分段方式，比如在时间表中增加一个业务ID，然后根据这个biz\_id做分片也可以。
...
[42.场景题_为啥不要在事务中做外部调用](../42.场景题/为啥不要在事务中做外部调用.md)

[50.项目难点&亮点_基于Spring_Event,实现同步转异步,解决定时任务扫表导致数据库连接池不够的问题](../50.项目难点&亮点/基于Spring_Event,实现同步转异步,解决定时任务扫表导致数据库连接池不够的问题.md)
