...
![](./assets/操作系统的IO模型有哪些_-6.png)￼

## 异步IO模型

**调用aio\_read令内核把数据准备好，并且复制到用户进程空间后执行事先指定好的函数**。就像是，小J交代大堂经理把业务给办理好了就通知他来验收，在这个过程中小J可以去做自己的事情。这就是真正的异步IO。

![](./assets/操作系统的IO模型有哪些_-7.png)￼
...
[39.操作系统_同步、异步、阻塞、非阻塞怎么理解](./同步、异步、阻塞、非阻塞怎么理解.md)

## 如何理解select，poll，epoll？

[39.操作系统_如何理解select、poll、epoll](./如何理解select、poll、epoll.md)
