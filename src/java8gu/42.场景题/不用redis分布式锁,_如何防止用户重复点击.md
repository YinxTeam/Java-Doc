---
title: "不用redis分布式锁,_如何防止用户重复点击"
author: Hollis
category:
  - Java八股文
---

# 不用redis分布式锁,_如何防止用户重复点击
...
sequenceDiagram
  ...
  User->>Browser: 进入商品详情页
  Browser->>auth: 获取 token
  auth->>auth: 生成 token
  auth->>redis: 保存 token
  auth->>Browser: 返回 token
  User->>Browser: 创建订单
  Browser->>TokenFilter: 创建订单（携带 token）
  TokenFilter->>redis: token 校验
  alt 校验失败
  TokenFilter->>Browser: 下单失败
  else 校验成功
  TokenFilter->>redis: 删除 token
  TokenFilter->>threadLocal: 保存 token
  TokenFilter->>trade: 下单接口执行
  trade->>threadLocal: 获取 token，创建订单
```

![图表](./assets/不用redis分布式锁,_如何防止用户重复点击_-1.svg)
