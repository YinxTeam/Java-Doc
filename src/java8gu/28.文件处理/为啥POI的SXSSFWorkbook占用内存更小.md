---
title: "为啥POI的SXSSFWorkbook占用内存更小"
author: Hollis
category:
  - Java八股文
---
...
在这个方法中，数据会在 \_out.write(...) 调用时写入磁盘，这里的\_out其实就是一个写入磁盘文件的Writer，他的write方法就会把内容写入到临时文件中。

我尝试着在\_out初始化的地方，也就是：
...
![image.png](./assets/为啥POI的SXSSFWorkbook占用内存更小_-1.png)

感兴趣的也可以debug看一下这个临时文件的内容，其实他就是一个xml文件，然后写入的就是我们excel中的内容。

![image.png](./assets/为啥POI的SXSSFWorkbook占用内存更小_-2.png)
