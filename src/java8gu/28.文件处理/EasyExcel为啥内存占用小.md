...
EasyExcel 之所以能在处理大型 Excel 文件时保持较低的内存占用，主要归功于其**SAX解析设计（逐行读取+事件机制）**和**磁盘缓存策略**。（本文主要介绍EasyExcel的文件读取占用内存小的原因，文件写入其实和POI的 SXSSFWorkbook 原理一样的。直接看[28.文件处理_为啥POI的SXSSFWorkbook占用内存更小](./为啥POI的SXSSFWorkbook占用内存更小.md) 就可以了）
...
![image.png](./assets/EasyExcel为啥内存占用小_-1.png)
...
![image.png](./assets/EasyExcel为啥内存占用小_-2.png)
