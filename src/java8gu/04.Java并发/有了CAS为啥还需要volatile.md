...
[04.Java并发_CAS在操作系统层面是如何保证原子性的](./CAS在操作系统层面是如何保证原子性的.md)

> 这个问题有的时候也这么问：解决并发问题，用CAS就够了么？

其实，CAS只能保证原子性，即一个修改命令，以原子性的操作完成，中间不会被中断。但是并发编程中，我们要解决的问题还包括有序性和可见性。

[04.Java并发_什么是Java内存模型(JMM)](./什么是Java内存模型(JMM).md#dIOas)

因为，**CAS虽然提供了一种无锁的方式来原子地更新值，但它本身并不能保证变量修改的可见性**。也就是说，一个线程可能已经通过CAS成功地改变了一个变量的值，但这个新值可能还没有被其他线程所看到，因为这个值可能仅仅存储在当前线程的本地内存中。

> 这里可能有人还有疑惑，在[04.Java并发_CAS在操作系统层面是如何保证原子性的](./CAS在操作系统层面是如何保证原子性的.md) 中明明说"cmpxchg是基于 CPU 缓存一致性协议实现的，在多核 CPU 中，所有核心的缓存都是一致的。"这里咋又不保证可见性了呢？ 主要是因为这里把多核CPU的缓存之间的一致性和Java的多线程本地内存之间的一致性搞混了。即没有正确的理解MESI和JMM的作用。可以看下：[04.Java并发_有了MESI为啥还需要JMM](./有了MESI为啥还需要JMM.md)
...
[04.Java并发_volatile是如何保证可见性和有序性的](./volatile是如何保证可见性和有序性的.md)
